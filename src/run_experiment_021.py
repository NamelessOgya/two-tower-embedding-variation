"""
Plan 021 Runner: Qwen3-Embedding Scaling Benchmark (0.6B vs. 4B vs. 8B)
-----------------------------------------------------------------------
Evaluates Qwen3 LLM-based embedding scale progression:
  - mE5-base (768-dim, RoBERTa baseline)
  - mE5-large (1024-dim, RoBERTa baseline)
  - Qwen3-0.6B (1024-dim, ~0.6B params)
  - Qwen3-4B   (2560-dim, ~4.0B params)
  - Qwen3-8B   (4096-dim, ~8.0B params)
...
def plot_plan_021_results(df: pd.DataFrame, out_path: Path):
    fig, axes = plt.subplots(2, 2, figsize=(15, 11))
    fig.suptitle("Plan 021: Qwen3-Embedding Scaling Benchmark (0.6B vs. 4B vs. 8B)\n(MovieLens 1M: 5 Seeds Mean ± Std Error Bars)", fontsize=15, fontweight='bold')

    palette = {
        "mE5-base (768d)": "#7f8c8d",
        "mE5-large (1024d)": "#34495e",
        "Qwen3-0.6B (1024d)": "#f39c12",
        "Qwen3-4B (2560d)": "#e67e22",
        "Qwen3-8B (4096d)": "#d35400",
    }
...
def main():
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("report/plan_021"))
    ...
    models_to_test = [
        ...
        ("Qwen3-4B (2560d)",   args.scales_dir / "user_embeddings_qwen3_4b.npy",   args.scales_dir / "item_embeddings_qwen3_4b.npy"),
        ("Qwen3-8B (4096d)",   args.scales_dir / "user_embeddings_qwen3_8b.npy",   args.scales_dir / "item_embeddings_qwen3_8b.npy"),
    ]
    ...
    csv_path = args.out_dir / "results_021.csv"
    df_results.to_csv(csv_path, index=False)
    log.info(f"Saved Plan 021 results CSV to {csv_path}")

    plot_path = args.out_dir / "tradeoff_plan_021.png"
    plot_plan_021_results(df_results, plot_path)

"""
Plan 022 Runner: F2LLM-v2 Multi-Scale & Cross-Family Benchmark
--------------------------------------------------------------
Compares 3 major embedding model families on downstream TwoTowerLogQInfoNCE:
  - mE5-large (1024d, RoBERTa)
  - Qwen3-0.6B (1024d), Qwen3-4B (2560d), Qwen3-8B (4096d)
  - F2LLM-0.6B (1024d), F2LLM-4B (2560d), F2LLM-8B (4096d)

Runs 5 seeds across accuracy and aggregate diversity metrics.
"""
...
def plot_plan_022_results(df: pd.DataFrame, out_path: Path):
    fig, axes = plt.subplots(2, 2, figsize=(16, 12))
    fig.suptitle("Plan 022: F2LLM vs. Qwen3 vs. mE5 Benchmark (MovieLens 1M)\n(5 Seeds Mean ± Std Error Bars)", fontsize=15, fontweight='bold')
    ...
    colors = [family_colors.get(f, "#3498db") for f in df["family"]]
    ...
    ax.set_xticks(x_pos)
    ax.set_xticklabels(df["model"], rotation=20, ha='right', fontweight='bold', fontsize=8.5)
    ax.set_title("(a) Recall@10 vs. Model Architecture ↑", fontweight='bold', fontsize=11)
    ax.set_ylabel("Recall@10", fontsize=10)
    ax.grid(True, alpha=0.3, axis='y')
    for i, v in enumerate(df["recall_10_mean"]):
        ax.text(i, v / 2, f"{v:.4f}", ha='center', va='center', color='white', fontweight='bold', fontsize=9)
    ...
    ax.set_xticks(x_pos)
    ax.set_xticklabels(df["model"], rotation=20, ha='right', fontweight='bold', fontsize=8.5)
    ax.set_title("(b) Precision@10 (%) vs. Model Architecture ↑", fontweight='bold', fontsize=11)
    ax.set_ylabel("Precision@10 (%)", fontsize=10)
    ax.grid(True, alpha=0.3, axis='y')
    for i, v in enumerate(df["precision_10_mean"]):
        ax.text(i, v / 2, f"{v:.2f}%", ha='center', va='center', color='white', fontweight='bold', fontsize=9)
    ...
    ax.set_xticks(x_pos)
    ax.set_xticklabels(df["model"], rotation=20, ha='right', fontweight='bold', fontsize=8.5)
    ax.set_title("(c) Catalog Coverage@10 (%) vs. Model Architecture", fontweight='bold', fontsize=11)
    ...
    for i, row in df.iterrows():
        c = colors[i]
        marker = 'o' if row["family"] == "Qwen3" else ('s' if row["family"] == "F2LLM" else '^')
        ax.errorbar(
...
def main():
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("report/plan_022"))
    ...
    models_to_test = [
        ("mE5-large (1024d)",   "mE5",   args.scales_dir / "user_embeddings_large.npy",       args.scales_dir / "item_embeddings_large.npy"),
        ("Qwen3-0.6B (1024d)", "Qwen3", args.scales_dir / "user_embeddings_qwen3_0p6b.npy",   args.scales_dir / "item_embeddings_qwen3_0p6b.npy"),
        ("Qwen3-4B (2560d)",   "Qwen3", args.scales_dir / "user_embeddings_qwen3_4b.npy",     args.scales_dir / "item_embeddings_qwen3_4b.npy"),
        ("Qwen3-8B (4096d)",   "Qwen3", args.scales_dir / "user_embeddings_qwen3_8b.npy",     args.scales_dir / "item_embeddings_qwen3_8b.npy"),
        ("F2LLM-0.6B (1024d)", "F2LLM", args.scales_dir / "user_embeddings_f2llm_0p6b.npy",   args.scales_dir / "item_embeddings_f2llm_0p6b.npy"),
        ("F2LLM-4B (2560d)",   "F2LLM", args.scales_dir / "user_embeddings_f2llm_4b.npy",     args.scales_dir / "item_embeddings_f2llm_4b.npy"),
        ("F2LLM-8B (4096d)",   "F2LLM", args.scales_dir / "user_embeddings_f2llm_8b.npy",     args.scales_dir / "item_embeddings_f2llm_8b.npy"),
    ]

    all_summaries = []
    for scale_name, family, u_path, i_path in models_to_test:
        ...
        summary = evaluate_scale_5seeds(
            scale_name, family, user_embs, item_embs, train_pos, test_pos, device=args.device
        )
        all_summaries.append(summary)

    df_results = pd.DataFrame(all_summaries)
    csv_path = args.out_dir / "results_022.csv"
    df_results.to_csv(csv_path, index=False)
    log.info(f"Saved Plan 022 results CSV to {csv_path}")

    plot_path = args.out_dir / "tradeoff_plan_022.png"
    plot_plan_022_results(df_results, plot_path)

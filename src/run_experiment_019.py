"""
Plan 019 Runner: Multi-Scale Embedding Benchmark (mE5-small / base / large)
---------------------------------------------------------------------------
Compares text embedding model scales on downstream TwoTowerLogQInfoNCE:
  - mE5-small (384-dim, ~118M params)
  - mE5-base  (768-dim, ~278M params)
  - mE5-large (1024-dim, ~560M params)

Runs 5-seed evaluation on single-trial recommendation metrics.
"""
...
from src.model.models_018 import TwoTowerLogQInfoNCE, TwoTowerBPRBase
...
def evaluate_scale_5seeds(
    ...
    for seed in SEEDS:
        ...
        # In_dim adapts dynamically to 384, 768, or 1024 inside prepare()
...
def plot_plan_019_results(df: pd.DataFrame, out_path: Path):
    fig, axes = plt.subplots(2, 2, figsize=(14, 11))
    fig.suptitle("Plan 019: Multilingual E5 Model Scale Benchmark (MovieLens 1M)\n(5 Seeds Mean ± Std Error Bars)", fontsize=15, fontweight='bold')

    colors = ["#3498db", "#2ecc71", "#e74c3c", "#9b59b6"]

    # (a) Recall@10 vs Model Scale
    ax = axes[0, 0]
    ...
    ax.set_title("(a) Recall@10 vs. Model Scale ↑", fontweight='bold', fontsize=11)
    ax.set_ylabel("Recall@10", fontsize=10)
    ax.grid(True, alpha=0.3, axis='y')
    for i, v in enumerate(df["recall_10_mean"]):
        ax.text(i, v / 2, f"{v:.4f}", ha='center', va='center', color='white', fontweight='bold', fontsize=11)

    # (b) Precision@10 (%) vs Model Scale
    ax = axes[0, 1]
    ax.bar(
        x_pos,
    ...
    ax.set_xticks(x_pos)
    ax.set_xticklabels(df["model"], fontweight='bold')
    ax.set_title("(b) Precision@10 (%) vs. Model Scale ↑", fontweight='bold', fontsize=11)
    ax.set_ylabel("Precision@10 (%)", fontsize=10)
    ax.grid(True, alpha=0.3, axis='y')
    for i, v in enumerate(df["precision_10_mean"]):
        ax.text(i, v / 2, f"{v:.2f}%", ha='center', va='center', color='white', fontweight='bold', fontsize=11)

    # (c) Catalog Coverage@10 (%) vs Model Scale
    ax = axes[1, 0]
    ax.bar(
        x_pos,
        df["coverage_10_mean"],
    ...
    ax.set_xticks(x_pos)
    ax.set_xticklabels(df["model"], fontweight='bold')
    ax.set_title("(c) Catalog Coverage@10 (%) vs. Model Scale", fontweight='bold', fontsize=11)
    ...
    for i, row in df.iterrows():
        ...
        ax.errorbar(
            ...
            capsize=5,
            markersize=10,
            label=f"{row['model']} ({int(row['in_dim'])}d)",
        )
...
def main():
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("report/plan_019"))
    ...
    scales_dir = Path("data/processed_scales")
    scales = [
        ("mE5-small (384d)", scales_dir / "user_embeddings_small.npy", scales_dir / "item_embeddings_small.npy"),
        ("mE5-base (768d)",  args.data_dir / "user_embeddings.npy",       args.data_dir / "item_embeddings.npy"),
        ("mE5-large (1024d)", scales_dir / "user_embeddings_large.npy", scales_dir / "item_embeddings_large.npy"),
    ]

    all_summaries = []
    for scale_name, u_path, i_path in scales:
    ...
    csv_path = args.out_dir / "results_019.csv"
    df_results.to_csv(csv_path, index=False)
    log.info(f"Saved Plan 019 results CSV to {csv_path}")

    plot_path = args.out_dir / "tradeoff_plan_019.png"
    plot_plan_019_results(df_results, plot_path)

"""
Plan 015 Experiment Runner: End-to-End Trained Multi-Head Stratified Partition Evaluation
-----------------------------------------------------------------------------------------
Evaluates the end-to-end trained multi-head partition model (TwoTowerTrainedMultiHeadStratifiedPartition)
against Base, Random Item Partition, and Semantic Stratified Partition.
...
Usage:
    PYTHONPATH=. python3 src/run_experiment_015.py --device cuda
"""
...
from src.model.models_015 import TwoTowerTrainedMultiHeadStratifiedPartition
...
N_SEEDS  = len(SEEDS)
OUT_DIR  = Path("report/plan_015")
DATA_DIR = Path("data/processed/movielens")


def run_experiment_015(model, test_gt, train_pos, user_embs, item_embs, item2genres, iid2idx, device):
...
def plot_plan_015_tradeoff(all_results: dict, out_dir: Path):
    out_dir.mkdir(parents=True, exist_ok=True)

    fig, axes = plt.subplots(2, 2, figsize=(16, 12), facecolor="#0f1117")
    fig.suptitle("Plan 015: End-to-End Trained Multi-Head Stratified Partition Evaluation",
    ...
    models_order = [
        ...
        "TT_trained_multihead_stratified_partition_n10",
    ]
    labels = [
        "Base (no-div)",
        "Random Partition",
        "Semantic Stratified",
        "Trained MultiHead Partition",
    ]
    colors = ["#555555", "#3498db", "#2ecc71", "#e74c3c"]
    ...
    plot_path = out_dir / "tradeoff_plan_015.png"
...
def main():
    ...
    models = [
        ...
        ("TT_trained_multihead_stratified_partition_n10", TwoTowerTrainedMultiHeadStratifiedPartition(base_tt, n_trials=10, n_clusters=20, epochs=25, lr=2e-3)),
    ]

    all_results = {}
    for name, model in models:
        log.info(f"\n============================================================\nEvaluating Model: {name}\n============================================================")
        res = run_experiment_015(model, test_gt, train_pos, user_embs, item_embs, item2genres, iid2idx, args.device)
        all_results[name] = res

    # Save Results
    with open(OUT_DIR / "results_015.json", "w") as f:
    ...
    df_metrics.to_csv(OUT_DIR / "results_015.csv", index=False)
    log.info(f"Saved CSV -> {OUT_DIR / 'results_015.csv'}")

    plot_plan_015_tradeoff(all_results, OUT_DIR)

    log.info("\n" + "="*70 + "\nPlan 015 Verification Summary\n" + "="*70)
    for name, res in all_results.items():
        m = res["mean"]
        log.info(
    ...
    log.info("\n✅ Plan 015 evaluation completed successfully!")

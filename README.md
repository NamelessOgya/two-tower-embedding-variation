...
## 📈 2 段階 Plackett–Luce におけるトレードオフ曲線と制御パラメータ
...
### Plackett-Luce (PL) モデルが持つ 2 つの制御ノブ

PL モデルでは、以下の **2 つの明快なハイパーパラメータ** によってトレードオフ曲線を精密にコントロール可能である。

#### ノブ 1: 温度パラメータ $\tau$ (Softmax Temperature)
- **$\tau \to 0$ ($\tau = 0.001$)**: Gumbel ノイズの影響が消滅し、**100% 決定論的 Top-$K$ FAISS 抽出**へ収束（Base モデルと一致、多様性 0.0）。
- **$\tau \in [0.1, 5.0]$**: 高い精度を維持したまま、試行ごとの表示バリエーション（Diversity $0.93 \to 0.95$）を滑らかに増加させる Pareto 最適な曲線を形成。
- **$\tau \to \infty$ ($\tau = 100.0$)**: 候補プール $\mathcal{C}_u$ 内での一様ランダムサンプリングへ移行。

#### ノブ 2: 候補プールサイズ $M$ (Stage 1 FAISS 検索深さ)
- **$M = K = 10$**: 候補数が Top-10 に制限されるため、どの $\tau$ を選んでも Top-10 内のシャッフルとなり候補拡張は起きない。
- **$M = 200$ (推奨値)**: 上位 5% の関連候補群だけに探索を絞り、下位 95% の非関連品を遮断するため、**多様性を出しても精度が落ちず、全スロット精度および Total Recall の両方で最高値を獲得（右上向きのトレードオフ曲線）**。
- **$M \to \text{全カタログ数}$ ($M=32,496$)**: Stage 1 フィルタが消失し、全カタログに対する単純ノイズ手法へ退化する。
...
### 2. 準上位（11〜200位）にある真の適合品の「掘り起こし」
- Two-Tower の埋め込み自体はユーザーの嗜好（ジャンルや意味空間）を正しく学習しており、**Top-11 〜 Top-200 の近傍候補プールの中には、そのユーザーに適合する正解アイテムが豊富に含まれている**。
- **Item Partitioning（物理的な排除）**: カタログを 10 分割することで Hub が 1 つの Bucket に封じ込められ、残り 9 試行では Hub が物理的に存在しないため、11〜200 位の適合品が強制的にスロットへ押し出されてヒットする。
- **2 段階 Plackett–Luce（確率的な排除 & ノイズ遮断）**: Stage 1 で下位 95% の無関係なゴミを遮断し、上位 $M=200$ 件の適合プールから Gumbel ノイズで確率サンプリングすることで、Hub の固定独占を解き、Top-200 内の適合アイテムを試行ごとにスロットへ流入させる。

### 3. カタログ規模拡大による Hubness 激化の定量的一致
- カタログが約 10 倍（3,706 件 ➔ 32,496 件）に拡大した Yelp では、高次元空間での Hub 集中が幾何学的にさらに激化するため、決定論的 Base の精度沈下が深刻になる。
- その結果、Plackett-Luce によるスロット解放時の Precision 改善幅は、MovieLens の **+42.4%** に対し、Yelp では **+78.1%** へと劇的に跳ね上がっている。

### 4. 学術研究（RecSys / SIGIR / NeurIPS / ICML）との整合性
